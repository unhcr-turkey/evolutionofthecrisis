...
d3.json("lib/statesme.json", function(error, tr) {
  map.selectAll("path")
      .data(topojson.feature(tr, tr.objects.counties).features)
      .enter()
      .append("path")
   ...
   map.append("path")
       .datum(topojson.mesh(tr, tr.objects.counties, function(a, b) { return a !== b; }))
       .attr("class", "province")
       .attr("vector-effect","non-scaling-stroke")
       .attr("d", path);
  ...
  map.append("path")
      .datum(topojson.mesh(tr, tr.objects.states, function(a, b) { return a === b && a.id === "TUR"; }))
      .attr("d", path)
      .attr("class", "country");
    
  svg.selectAll(".country-label")
    .data(topojson.feature(tr, tr.objects.states).features)
    .enter().append("text")
  ...
  d3.csv("lib/SyrianRefPopulation.csv",function(data){
    var first = data[0];
    // get columns
...
function createLegend(){
  var legend = g.append("g").attr("id","legend").attr("transform","translate(50,10)");

  legend.append("circle").attr("class","gain").attr("r",5).attr("cx",150).attr("cy",520)
  legend.append("circle").attr("class","loss").attr("r",5).attr("cx",150).attr("cy",540)

  legend.append("text").text("Syrian Refugees Registered in Camps").attr("x",160).attr("y",524);
  legend.append("text").text("Syrian Refugees Registered in Urban").attr("x",160).attr("y",544);
  ...
  for ( var i in sizes ){
    legend.append("circle")
      ...
      .attr( "cy", 8.2 * circleSize( sizes[sizes.length-1] ) - circleSize( sizes[i] ) )
      .attr("vector-effect","non-scaling-stroke");
    legend.append("text")
      .text( (sizes[i] / 1000) + "K" + (i == sizes.length-1 ? " Refugees" : "") )
      .attr( "text-anchor", "middle" )
      .attr( "x", 1 + circleSize( sizes[sizes.length-1] ) )
...
function setProbeContent(d){
  ...
  var html = "<strong>" + d.CITY + "</strong><br/>" +
            format( Math.abs( val ) ) + " Syrian Refugees " + ( val < 0 ? "in Province" : "in Camps" ) + "<br/>" +
